...
class CGenerator:
    def __init__(self):
        ...
        self.requires_stdio = False
        self.requires_stdbool = False
    ...
    def generate(self, root: PseudoNode) -> str:
        ...
        has_main = any(child.type == "function_definition" and child.child_by_field_name('declarator').child_by_field_name('declarator').text.decode('utf8') == "main" for child in root.named_children)
        ...
        imports = []
        if self.requires_stdio:
            imports.append("#include <stdio.h>")
        if self.requires_stdbool:
    ...
    def _infer_type(self, node: PseudoNode) -> str:
        if node._semantic_type and node._semantic_type.name != "Unknown":
            if node._semantic_type.name == "Int": return "int"
            elif node._semantic_type.name == "String": return "char*"
            elif node._semantic_type.name == "Bool":
                self.requires_stdbool = True
                return "bool"
            elif node._semantic_type.name == "Float": return "double"
    ...
    def _visit(self, node: PseudoNode):
        if node.type == NODE_ASSIGNMENT:
            ...
            if left_str not in self.declared_vars:
                # Need explicit type in C
                c_type = self._infer_type(right)
                self._add_line(f"{c_type} {left_str} = {right_str};")
            ...
            if func_str == "printf":
                ...
                self._add_line(f"printf({args_str});")
            else:
                args_str = ""
                if args:
                    args_list = [self._expr(arg) for arg in args.named_children]
