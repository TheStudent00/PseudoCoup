...
class CppGenerator:
    ...
    def generate(self, root: PseudoNode) -> str:
        ...
        output = "\n".join(self.lines)
        if self.requires_iostream:
            output = output.replace("/* IMPORTS_PLACEHOLDER */", "#include <iostream>")
        else:
    ...
    def _visit(self, node: PseudoNode):
        if node.type == NODE_ASSIGNMENT:
            ...
            if left_str not in self.declared_vars:
                # Use auto for assignments
                self._add_line(f"auto {left_str} = {right_str};")
            ...
            if func_str == "std::cout":
                args_str = ""
                if args:
                    args_list = [self._expr(arg) for arg in args.named_children]
                    args_str = " << ".join(args_list)
                self._add_line(f"std::cout << {args_str} << std::endl;")
            else:
                args_str = ""
                if args:
                    args_list = [self._expr(arg) for arg in args.named_children]
