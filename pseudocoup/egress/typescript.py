    ...
    def _visit(self, node: PseudoNode):
        if node.type == NODE_ASSIGNMENT:
            ...
            if left_str not in self.declared_vars:
                # Use let for assignments
                type_str = ""
                if node._semantic_type and node._semantic_type.name != "Unknown":
                    # Convert Universal types to TypeScript
                    if node._semantic_type.name == "Int": type_str = ": number"
                    elif node._semantic_type.name == "String": type_str = ": string"
                    elif node._semantic_type.name == "Bool": type_str = ": boolean"
                    elif node._semantic_type.name == "Float": type_str = ": number"
                
                if type_str:
                    self._add_line(f"let {left_str}{type_str} = {right_str};")
                else:
                    self._add_line(f"let {left_str} = {right_str};")
