...
class PythonToTsTranspiler(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node):
        ...
        if init_method:
            ...
            for arg in init_method.args.args:
                ...
                ts_type = self.map_type(arg.annotation)
                params.append(f"{arg.arg}: {ts_type}")
                
            self.add_line(f"constructor({', '.join(params)}) {{")
            self.indent_level += 1
    ...
    def visit_FunctionDef(self, node):
        ...
        prefix = "" if self.current_class else "function "
        self.add_line(f"{prefix}{node.name}({', '.join(params)}): {ret_type} {{")
        self.indent_level += 1
        for body_node in node.body:
            self.visit(body_node)
        self.indent_level -= 1
    ...
    def visit_AnnAssign(self, node):
        ...
        value = self.get_expr_str(node.value) if node.value else "null"
        ts_type = self.map_type(node.annotation)
        self.add_line(f"let {target}: {ts_type} = {value};")
    ...
    def visit_For(self, node):
        ...
        if is_cstyle:
            self.add_line(f"for (let {var_name} = {start_val}; {var_name} < {end_val}; {var_name}++) {{")
        else:
            self.add_line(f"for (let {var_name} of Array.from({{length: {end_val} - {start_val}}}, (_, i) => i + {start_val})) {{")
    ...
    def get_expr_str(self, expr_node):
        if isinstance(expr_node, ast.Attribute):
            ...
            if func_name == "print":
                if not args: return "console.log()"
...
def transpile(code: str) -> str:
    import ast
    tree = ast.parse(code)
    transpiler = PythonToTsTranspiler(ledger={})
