...
class PythonToDartTranspiler(ast.NodeVisitor):
        ...
        if isinstance(node, ast.Name):
            ...
            if base == "Optional":
                ...
                if isinstance(node.slice, ast.Tuple):
                    k = self.map_type(node.slice.elts[0])
                    v = self.map_type(node.slice.elts[1])
                    return f"Map<{k}, {v}>"
    ...
    def visit_ClassDef(self, node):
            ...
            if isinstance(child, ast.FunctionDef) and child.name == "__init__":
                ...
                for stmt in child.body:
                    if isinstance(stmt, ast.AnnAssign) and isinstance(stmt.target, ast.Attribute) and isinstance(stmt.target.value, ast.Name) and stmt.target.value.id == "self":
                        ...
                        ts_type = self.map_type(stmt.annotation)
                        self.properties.append({"name": target, "type": ts_type})

        self.add_line(f"class {self.current_class} {{")
        self.indent_level += 1
        
        for p in self.properties:
            self.add_line(f"late {p['type']} {p['name']};")
        ...
        if init_method:
            ...
            for arg in init_method.args.args:
                ...
                dart_type = self.map_type(arg.annotation)
                params.append(f"{dart_type} {arg.arg}")
                
            self.add_line(f"{self.current_class}({', '.join(params)}) {{")
            self.indent_level += 1
    ...
    def visit_FunctionDef(self, node):
        ...
        self.add_line(f"{ret_type} {node.name}({', '.join(params)}) {{")
        self.indent_level += 1
        for body_node in node.body:
            self.visit(body_node)
        self.indent_level -= 1
    ...
    def visit_AnnAssign(self, node):
        ...
        # Don't add 'let' if it's already declared in properties
        ...
        value = self.get_expr_str(node.value) if node.value else "null"
        dart_type = self.map_type(node.annotation)
        self.add_line(f"{dart_type} {target} = {value};")
    ...
    def visit_For(self, node):
        ...
        for entry in self.ledger:
            if entry.get("ts_node") == "for(C-style)":
                is_cstyle = True
                
        if is_cstyle:
            self.add_line(f"for (int {var_name} = {start_val}; {var_name} < {end_val}; {var_name}++) {{")
        else:
            self.add_line(f"for (var {var_name} in Iterable.generate({end_val} - {start_val}).map((i) => i + {start_val})) {{")
    ...
    def get_expr_str(self, expr_node):
        if isinstance(expr_node, ast.Attribute):
            ...
            if isinstance(op_node, ast.Lt): op = "<"
                ...
                if right == "None": return f"{left} == null"
                return f"{left} == {right}"
...
def transpile(code: str) -> str:
    import ast
    tree = ast.parse(code)
    transpiler = PythonToDartTranspiler(ledger={})
