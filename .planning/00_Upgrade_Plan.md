...
> `version = "3.0.0"` -> `"4.0.0"` (rename approved by the owner in the executing session). Any doc
...
Status date: 2026-07-14. Author-of-record: planning agent, for the owner's sign-off. This is a
...
the exact question U5 answers for the owner:
...
whose output is a the owner decision, not code). U6 depends on U1-U4 landing and on U5's decision.
...
`pseudoir`'s OWN tests unless those sibling data dirs move too. Flag this to the owner under D1.
...
**Escalation triggers.**
- D1 (the owner): path-dependency vs vendored-copy — introduces new structure/a new vendoring
...
**Escalation triggers.**
- E2a (the owner + PseudoIR owner): several core arithmetic/identity operators PseudoCoup lowers (`//`,
...
- Any op-to-emitter mapping that requires a NEW UR-AST node type (user-type operators,
  destructuring, match — none are modeled in base `ur_ast.py` today) is a structure decision → the owner.
...
**Escalation triggers.**
- E4a (the owner): the gate is Hub-notation-specific but PseudoCoup ingests 12 languages. Deciding
  WHICH ingress paths the gate runs on (only Python-Hub? all? off by default with a flag?) is a
  behavior decision → the owner.
...
(upstream-to-base vs declare-app-specific) for the owner to ratify (decision D2). This unit's OUTPUT is a
decision-ready classified inventory plus, for the items the owner approves as upstream, the port itself.
...
Starting classification (from the survey; U5 refines and the owner ratifies):

| Vendored improvement | Where | Recommendation (for the owner) |
|---|---|---|
...
recommendation. For each item the owner ratifies as UPSTREAM, the port lands in base PseudoCoup and the
base's other-language emitters still produce identical output (verified by U6's round-trip on the
untouched languages). For each item the owner ratifies as APP-SPECIFIC, it is recorded as such (a note in
the inventory) and NOT copied into base. The vendored copy is not modified by this unit (base is the
target of upstreaming; the vendored copy stays as-is unless the owner says otherwise).

**Escalation triggers.**
- D2 (the owner): the upstream/app-specific split for EACH item is the owner's ratification. The table above is
  a recommendation, not a decision. Present it; do not port anything the owner has not approved.
- Any upstream port that changes base behavior for a language other than the one it came from
  ...
  scope with the owner before touching more than Kotlin.
...
- Repairing vs replacing `run_tests.sh` — if the cleanest fix is a full pytest rewrite (new test
  structure/files), that is new structure → confirm approach with the owner (small if it's a
...
## Decision queue — ALL DECIDED (the owner, 2026-07-14)

Executors: these are settled. Do not re-open them or re-present them as questions. Standing norm
from the owner, added to the delegation rules: when a question has a natural answer, DECIDE it, record
...
- SUPERSEDED (2026-07-14, by the owner's D1/D4 decisions — this bullet previously forbade modifying
...
- **No silent structural choices, but no re-litigating settled ones either**: any NEW file, name,
  ...
  D1-D5 are DECIDED (see the decision queue) — executors act on them without re-asking. the owner's
...
- `22_divergence_inventory.md` — (to be authored in U5) the classified base-vs-vendored divergence
  inventory with per-item upstream/app-specific recommendations for the owner.
