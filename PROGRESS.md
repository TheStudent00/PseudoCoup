...
|-------|------------|:------:|-------------------------|
| **Track B** — transpiler center | Kt→Py of the whole app + verification ladder. **The documented current priority.** | 🟡 | WFL_MixingCenter (parse-clean 192/254) is STALE `literal_transpiler` output. Measured via the canonical **KtToPy** (log_126): repositories **20/20 parse**, **17/20 construct (DI)**, **14/20 run real methods, 127/232 methods execute** — vs literal_transpiler's ~1/20. 2 transpiler bugs FIXED (elvis-`?:`-continue, `ifEmpty{continue}`), oracle 11/11. Remaining 14→20 = **runtime-shim coverage** (Flow ops like `flatMapLatest`), NOT transpiler bugs. Runnability ≠ correctness (oracle's separate ladder). |
