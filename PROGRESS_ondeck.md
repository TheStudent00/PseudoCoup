...
- [fidelity] 107/109 on 8 screens. Constraint tracking LANDED (build-time incoming-max threaded through to_widget; static ceiling joins the dynamic parent cap -- no oscillation) + lazy-list Item boundary nodes (list spacing between ITEMS only). ExerciseDetail's last 2 fails: a uniform +3.5% y accumulation above its final section (band-edge; likely small per-section padding differences) -- probe DetailSection's chain.
- [fidelity] Next screens: Wins, LogWorkout, Onboarding (needs CrashlyticsBridge stand-in); richer fixtures: a seeded program (Programs/Today list layouts).
