...
- [fidelity] 93/93 on 7 screens + specimen gate; History now measures a DATA-RICH card (seeded cardio session), which surfaced and fixed 4 content bugs (see shipped). Keep growing coverage: Wins, ExerciseDetail (nav args), LogWorkout, Onboarding; and richer fixtures (a seeded program for Programs/Today).
